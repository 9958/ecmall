...
class Payment extends BaseModel {
    ...
    public static function getList(array $attributes)
    {
        ...
        if (isset($userAgent['Platform']) && strtolower($userAgent['Platform']) == 'wechat') {
            ...
            if (self::where(['type' => 'ecmall_cert', 'status' => 1, 'code' => 'ecmall_service_cert_auth'])->first()) {
            ...
            $model = self::where(['type' => 'payment', 'status' => 1])->whereNotIn('code', ['wxpay.web', 'alipay.wap'])->get()->toArray();        
        }
    ...
    public static function pay(array $attributes)
    {
        ...
        $order = Order::where(['buyer_id' => $uid, 'order_id' => $order, 'status' => Order::ORDER_PENDING])->with('goods')->first();
        ...
        if ($code == 'teegon.wap') {
            ...
            $config = Pay::checkConfig($shop, 'yunqi');
            ...
            if (empty($config['yunqi_appkey']) || empty($config['yunqi_appsecret'])) {
            ...
            $srv = new TeegonService('https://api.teegon.com/', $config['yunqi_appkey'], $config['yunqi_appsecret']);
    ...
    public static function notify($code)
    {
        ...
        if ($code == 'teegon.wap') {
            ...
            if (isset($_POST['charge_id'])) {
                ...
                if($_POST['is_success'] == true){
                    ...
                    //业务代码
                    self::updateOrderPaid($order, '天工收银', $code);
                    return true;
                }
                else{
                    Log::info('notify:'. json_encode($_POST));
        ...
        if ($code == 'alipay.app' || $code == 'alipay.wap') {
            ...
            if($verify_result) {//验证成功
                ...
                if($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
                    ...
                    //业务代码
                    self::updateOrderPaid($order, '支付宝支付', $code);
        ...
        if ($code == 'wxpay.app') {
            ...
            if($resHandler->isTenpaySign() == true) {
                ...
                if ("SUCCESS"==$return_code){
                    ...
                    //业务代码
                    self::updateOrderPaid($order, '微信支付', $code);
        ...
        if ($code == 'unionpay.app') {
            ...
            if($unionpay->params['respCode'] == '00') {
                ...
                //业务代码
                self::updateOrderPaid($order, '银联支付', $code);
            }
            echo 'success';
            return true;
        }
    ...
    public static function updateOrderPaid($order, $name, $code)
    {
        $order->payment_name = $name;
